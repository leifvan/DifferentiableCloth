...
def train(args_expt, args_data):
	...
	print("init image loss")
	image_loss_object = ImageLoss(diff_renderer, args_expt, args_data, log, 'cpu')
	...
	for cur_iter in tqdm(range(start, args_expt.getint('max_iterations')), "loop"): 
		...
		print(f"backprop took {time() - start_time:.2f}secs")
		start_time = time()
		optimizer.step()					
		print(f"optimizing took {time() - start_time:.2f}secs")
